...
=============================================================================
Provides:
1. Micro-Patch Mode (Ultra-Fast 500KB - 1.5MB Hotfix Updates, Zero PC Freezes, 0.2s Restart)
...
class UpdateEngine:
    ...
    - Mode 1: Micro-Patch (1 MB Patch ZIP) -> 0.2s Restart, zero disk/CPU strain, zero PC lag
    - Mode 2: Full Binary (106 MB Standalone EXE) -> Used only for major framework version leaps
    """
    ...
    def check_for_updates(self, custom_url: Optional[str] = None) -> Dict[str, Any]:
        ...
        if not data and 'github.com' in feed_url:
            try:
                parts = feed_url.replace("https://raw.githubusercontent.com/", "").split("/")
                if len(parts) >= 2:
        ...
        update_type = data.get("type", "patch" if str(data.get("download_url", "")).lower().endswith(".zip") else "full")
        ...
        result = {
            ...
            "file_size": str(data.get("file_size", "850 KB" if update_type == "patch" else "106 MB")),
    ...
    def download_update_executable(
        ...
        - If ZIP (Micro-Patch ~1MB): Extracts directly to %APPDATA%/SKD_Tool/patches/
        ...
        is_zip = download_url.lower().endswith('.zip') or (self.cached_update_info and self.cached_update_info.get("type") == "patch")
                    ...
                    with open(target_path, 'wb') as f:
                        for chunk in resp.iter_content(chunk_size=65536):
                            ...
                            if chunk:
                                ...
                                if elapsed >= 0.15 or downloaded == total_bytes:
        ...
        if progress_callback:
            progress_callback({
                ...
                "total": os.path.getsize(target_path) if os.path.exists(target_path) else 0,
                "speed": 0,
                "eta": 0
            })
    ...
    def launch_update_and_exit(self, new_file_path: str):
        ...
        if is_micro_patch:
            ...
            if sys.platform == "win32":
                target_exec = current_exe if current_exe else [sys.executable, os.path.abspath(sys.argv[0])]
                if isinstance(target_exec, list):
                    subprocess.Popen(target_exec, creationflags=subprocess.DETACHED_PROCESS | subprocess.CREATE_NEW_PROCESS_GROUP)
                else:
                    subprocess.Popen([target_exec], creationflags=subprocess.DETACHED_PROCESS | subprocess.CREATE_NEW_PROCESS_GROUP)
            else:
        ...
        if sys.platform == "win32":
            ...
            if is_installer:
                subprocess.Popen([new_file_path], creationflags=subprocess.DETACHED_PROCESS | subprocess.CREATE_NEW_PROCESS_GROUP)
...
exit
""")
                subprocess.Popen(["cmd.exe", "/c", bat_path], creationflags=subprocess.DETACHED_PROCESS | subprocess.CREATE_NEW_PROCESS_GROUP)
            else:
                subprocess.Popen([new_file_path], creationflags=subprocess.DETACHED_PROCESS | subprocess.CREATE_NEW_PROCESS_GROUP)
        else:
